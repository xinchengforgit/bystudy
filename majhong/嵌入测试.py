...
async def out_card(app:GraiaMiraiApplication,friend:Friend,message:MessageChain):
    ...
    if run_game==True and friend.id==playerqueue[cur]:
            ...
            #             At(playerqueue[i]),
            #         Plain(":"+str(playerlist[i])+"\n")])
            #         messagechain1.join(tempchain)
            # app.sendGroupMessage(groupqueue[0],MessageChain.create([
            #     At(playerqueue[cur]),
            #     Plain("打出了 "+message_str),
            # ]))
            # app.sendGroupMessage(groupqueue[0],messagechain1)
